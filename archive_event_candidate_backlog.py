#!/usr/bin/env python3
"""Archive only the reviewed low-priority DynamoDB event-candidate rows.

The command is fail-closed: it derives keys from the same dry-run report,
writes a restore plan before updates, and conditionally updates only rows that
still have their reviewed status.  It never deletes data or touches inboxes.
"""
