#!/usr/bin/env python3
"""Apply reviewed YouTube short/video evidence to existing event pages."""
...
from notion_support.notion_config import EVENT_DATA_SOURCE_ID, load_local_env


REVIEW = Path("data/youtube_active_video_review.json")
OUT = Path("data/youtube_review_video_evidence_apply_result.json")
OUT_MD = Path("data/youtube_review_video_evidence_apply_result.md")

JIYUGAOKA_EVENT_NAME = "自由が丘納涼盆踊り大会"
MARUNOUCHI_EVENT_NAME = "丸の内de盆踊り"
SHIBUYA_EVENT_NAME = "渋谷盆踊り2025"
...
def review_rows(review):
    return [row for row in review.get("rows") or [] if row.get("action") == "review_video_evidence"]
...
def classify_row(row):
    ...
    if "自由が丘" in haystack:
        return {
            ...
            "target_event_name": JIYUGAOKA_EVENT_NAME,
            "reason": "既存イベントの曲目/動画補助証拠として利用できる",
        }
    if "丸の内" in haystack or "Marunouchi" in haystack:
        return {
            "decision": "append_existing_event",
            "target_event_name": MARUNOUCHI_EVENT_NAME,
            "reason": "公式確認済みイベントの短尺動画補助証拠として利用できる",
        }
    if "渋谷" in haystack or "Shibuya" in haystack:
        return {
            "decision": "hold",
            "target_event_name": SHIBUYA_EVENT_NAME,
            "reason": "公式確認が未解決のため、YouTube短尺動画単独では登録/追記しない",
        }
    return {
        "decision": "hold",
        "target_event_name": "",
...
def evidence_note(event_name, rows):
    ...
    lines = [
        "[youtube_evidence] YouTube shorts追加証拠",
        f"- 対象イベント: {event_name}",
...
def build_updates(api, review):
    grouped, held = grouped_ready(review_rows(review))
    updates = []
...
def render_markdown(output):
    lines = [
        "# YouTube review video evidence apply結果",
        "",
...
def main():
    ...
    try:
        require_confirmation(
            ...
            "legacy YouTube review-video Notion update",
        )
    ...
    output = {
        "generated_by": "apply_youtube_review_video_evidence.py",
