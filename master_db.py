"""Helpers for the bon odori master SQLite database."""
...
MASTER_SCHEMA = DATA / "bon_odori_master.schema.sql"
MASTER_MANIFEST = DATA / "bon_odori_master_manifest.json"
...
def init_db(path=MASTER_DB, force_rebuild_from_snapshot=False):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        if not force_rebuild_from_snapshot:
            raise SystemExit(
                f"Cannot rebuild from snapshot: {path} already exists. "
                "Use --force-rebuild-from-snapshot to delete and rebuild it from source snapshots."
            )
        path.unlink()
