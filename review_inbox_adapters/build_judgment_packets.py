...
def run(args):
    ...
    excluded_by_reason={reason:sum(1 for item in excluded if item["reason"]==reason) for reason in ("superseded","expired","not_eligible","claimed_by_other")}
    report={"generated":len(packets),"batches":batches,"excluded":excluded,"excluded_by_reason":excluded_by_reason,"waiting_count":max(0,len(rows)-len(packets)-len(excluded)),"migrations_applied":migrations,"claim_scope":"production" if args.apply else "dry_run_copy","force_claim_used":bool(force_claimed),"force_claimed_inbox_ids":force_claimed}
    args.report_json.parent.mkdir(parents=True,exist_ok=True)
    args.report_json.write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n")
    return report
def main():
 p=argparse.ArgumentParser();p.add_argument('--db',type=Path,default=MASTER_DB);p.add_argument('--out-db',type=Path,default=Path('data/judgment_packets_dry_run.sqlite'));p.add_argument('--out-dir',type=Path,default=Path('data/judgment_packets'));p.add_argument('--report-json',type=Path,default=Path('data/judgment_packets_report.json'));p.add_argument('--actor-id');p.add_argument('--batch-size',type=int,default=20);p.add_argument('--max-packets',type=int,default=100);p.add_argument('--lease-minutes',type=int,default=30);p.add_argument('--force-claim',action='store_true');p.add_argument('--domain',default='event');p.add_argument('--apply',action='store_true');p.add_argument('--confirm',default='');p.add_argument('--no-auto-migrate',action='store_true');a=p.parse_args();print(json.dumps(run(a),ensure_ascii=False));return 0
