...
def make_packet(row,when):
    ...
    retry=[] if not occ else [{"candidate_id":stable_id("retry",row["inbox_id"],"v1"),"next_eligible_at":(when+timedelta(days=14)).isoformat(),"window_start":(when+timedelta(days=7)).isoformat(),"window_end":min(when+timedelta(days=30),datetime.fromisoformat(row["expires_at"])) .isoformat(),"occurrence_ids":occ,"evidence_ids":ev,"retrieved_at":when.isoformat(),"calculation_version":"retry-window/v1","input_hash":sha256_hex({"inbox_id":row["inbox_id"],"occurrence_ids":occ,"evidence_ids":ev})}]
    return {"packet_version":1,"packet_id":pid,"inbox_id":row["inbox_id"],"domain":row["contract_domain"],"lane":row["contract_lane"],"source_id":row["source_id"],"source_key":row["source_key"],"source_payload_hash":row["source_payload_hash"],"generated_at":when.isoformat(),"expires_at":row["expires_at"],"proposal":p["proposal"],"targets":p["targets"],"resolved_target":p.get("resolved_target"),"evidence":[{"evidence_id":x,"source_url":row["source_url"],"excerpt":p.get("raw_excerpt",""),"retrieved_at":p["targets"]["retrieved_at"]} for x in ev],"retry_candidates":retry,"retry_unavailable_reason":None if retry else "no_occurrence_candidates","allowed_actions":allowed(row["contract_domain"],row["contract_lane"]),"reason_codes":REASON_CODE_HOLD_MODE}
def run(args):
