...
修正なしの比較結果や公開承認へ読み替えない。

本番反映前にPR #265をmergeし、正本をrun `34340430748` で再取得した。
そのcapture commit `e90134824f81115d27689e83177b256ad74c63e2`（年越し修正済み、R2前）と
R2統合後を追加patchなしで比較し、`original_parity` でも9ケース・全4出力がbyte一致した。
比較したPythonソース630ファイルはR2 merge commit `30a743f` と一致する。
比較JSON SHA-256: `b939260a332a68ff7177868625c3b2d36fede2622f53c29c13513bfc5ef2f65a`。
新しい入力組による合格であり、以前の入力組と旧commitでの拒否記録は保存する。
