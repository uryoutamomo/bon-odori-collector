...
この二つのruntimeは、似ていても更新失敗時の振る舞いが異なる。
別名runtimeはRDB欠落・空表を既存データで明示的に保護する。
用語runtimeはNotionから読めないと空の辞書を返すため、日次artifactの件数を見る必要がある。
両者を同じ「辞書だから安全」と扱わない。

語の別名とイベント／会場別名も同じではない。
前者は投稿の分類・曲候補・表示上の解釈に使う。
後者はcanonicalな系列または物理会場を決める照合の手掛かりである。
型を失った一般aliasを地理的根拠へ流用しない。
...
RDBの表が空だった理由は、同じではない。
`event_series_aliases` が無いなら、古いRDBに系列別名移行をまだ適用していない状態である。
表が存在して空なら、移行直後でseedが未投入の可能性がある。
runtime builderは理由を区別して残し、次に使うべき手動入口も表示する。
...
手動fallbackは、日次の失敗を隠すための常設代替ではない。
ローカルで作ったreview artifactはGitHub Actionsが見ない未レビュー差分になり得る。
そのため運用書は、日次が使えず、内田さんが即時のreview UIを必要としたときだけ使うよう定めている。

`run_series_alias_migration.py` もworkflowから呼ばれない手動入口である。
通常はdry-runで、実DBへ表を作る操作は確認句を伴う。
日次runtimeの欠落を見つけても、この移行を自動実行して埋めることはしない。
...
用語集の変更も、Notionの行、日次runtime、収集時の判定という三段を通る。
候補を有効化する決定は最初の段で行う。
runtime JSONを直接編集して緊急回避しても、Notionの正本を直さなければ次の生成で消える。
逆にNotionだけ直しても、日次生成前のcommit済みruntimeを読む利用側にはすぐ届かない。
...
この仕様は、日次に動く二つのruntime builderを中心にしている。
周辺にあるexport、候補生成、UIは同じ用語を扱うが、稼働状態は別である。
新しい処理を足すときは、まずどちらの正本を読んでいるかを決める。
次に、候補生成・レビュー・正本書き込み・runtime生成のどの段を担当するかを明記する。
その区別が無い処理は、既存の安全境界をまたいでしまう。
仕様を変える前に、runtimeを読む利用側も必ず検索する。
その結果をこのL1へ反映する。
