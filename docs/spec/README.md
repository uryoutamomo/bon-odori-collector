...
町会掲示板・区報PDF・X投稿・YouTube概要欄には地域の情報が存在するが、構造化されていない。
「同じ行事か」「実在するか」「会場はどこか」を収集の一件ごとに判断する必要があり、
件数が多いため、人手だけで広く・継続的に整備するのは難しい。そのため、利用者がAIに聞いたときに
根拠をもって答えられる基盤は、十分に整備されてこなかった。

盆助は、AIを読解・分類・名寄せ・差分検出の労働力として使い、**人間には整理コストが
高すぎた散在情報を、AIの小さな判断の積み重ねで、出典付き・更新可能・AI可読な事実へ変える**
ことを中核に置く。単なる盆踊り検索サイトではなく、この変換そのものが盆助の実体である。

だから盆助は、正解だけを1行に確定させたレコードを持つのでは足りない。
情報源ごとの主張・根拠・確度・変更履歴を保持し、名寄せの履歴・判断理由・情報源の信頼特性
そのものを資産として蓄積する。AIが高確度な案件をこなし、人は曖昧な境界事例だけを裁定する
——盆踊りは、「不完全で矛盾した公開情報から、現実世界の状態を継続的に復元する」ことの
実証領域として位置づけている。

（2026-08-24、内田さんとおとの対話で言語化。以後、仕様・設計・優先順位を判断する基準にする）
