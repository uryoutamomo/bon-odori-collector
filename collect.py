...
_X_FIRST_PERSON_RE = re.compile(r"私|僕|俺|自分|わたし|うちの")
...
def _x_area_bot(row):
    ...
    return (
        ...
        and row.get("first_person_ratio", 0) <= 0.15
...
def _add_x_bonodorer_scores(accounts, text_metrics, gap_credits):
    for handle, row in accounts.items():
        ...
        if posts:
            ...
            row["first_person_ratio"] = round(_x_smoothed_ratio(row["first_person_count"], posts, 0.20), 4)
            ...
            row.update({
                ...
                "listy_ratio": 0.15, "opinion_ratio": 0.35, "first_person_ratio": 0.2,
        ...
        voice = 0.0
        if posts >= 3:
            voice = 10 * row["opinion_ratio"] * (0.5 + 0.5 * row["first_person_ratio"])
    ...
    for v in voices:
        ...
        if text:
            metrics = text_metrics.setdefault(handle, {
                ...
                "url_count": 0, "listy_count": 0, "opinion_count": 0, "first_person_count": 0,
            ...
            metrics["first_person_count"] += int(bool(_X_FIRST_PERSON_RE.search(text)))
