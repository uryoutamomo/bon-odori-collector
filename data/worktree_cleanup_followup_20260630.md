...
| metric | value |
| --- | ---: |
| tracked changed files | 51 |
| untracked files | 32 |
| tracked diff size | +103,160 / -455,174 |

## Public JSON status

The event URL mismatch and public song-occurrence switch are resolved, but do not deploy the remaining generated data diff as-is.
...
- The remaining deploy blockers are now non-song generated public guard/dry-run files plus broader YouTube/ops generated outputs.

Recommended next split:

1. Review non-song public guard/dry-run files (`public_events_sync_guard`, historical reference dry-run, season hint dry-run).
