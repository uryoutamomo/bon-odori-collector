"""Apply reviewed missing occurrence source_url candidates to the master RDB.

Default mode writes to a copied SQLite DB. Apply mode only fills source_url for
rows classified as ready_source_url_candidate; it does not write Notion or
public JSON, and it does not change dates, venues, or source_kind.
"""
...
from master_rdb.master_db import MASTER_DB, connect_existing, refresh_manifest_database_state, table_counts


DATA = Path("data")
REVIEW_JSON = DATA / "missing_source_url_review.json"
OUT_DB = DATA / "reviewed_missing_source_urls_dry_run.sqlite"
OUT_JSON = DATA / "reviewed_missing_source_urls_apply_report.json"
OUT_MD = DATA / "reviewed_missing_source_urls_apply_report.md"
BACKUP_DIR = DATA / "backups"
CONFIRM = "APPLY REVIEWED MISSING SOURCE URLS"
...
def occurrence(conn, occurrence_id):
    result = rows(
        ...
        SELECT o.occurrence_id, o.series_id, o.display_name, o.event_year,
               ...
               o.lifecycle_status, o.source_kind, o.source_url,
               s.canonical_name AS series_name, s.source_url AS series_source_url
...
def ready_items(review):
    return [
        ...
        if item.get("review_action") == "ready_source_url_candidate"
        and item.get("candidate_source_url")
    ]
...
def build_plan(conn, review):
    ...
    for item in ready_items(review):
        ...
        if not before:
            ...
            continue
        if before.get("source_url"):
            skipped.append({**item, "reason": "occurrence_already_has_source_url"})
            continue
        planned.append(
            {
                ...
                "old_source_url": before.get("source_url") or "",
                "new_source_url": item["candidate_source_url"],
                "candidate_source_kind": item.get("candidate_source_kind") or "",
...
def apply_plan(conn, planned, now):
    ...
    for item in planned:
        cursor = conn.execute(
            ...
            """,
            (item["new_source_url"], now, item["occurrence_id"]),
        )
        if cursor.rowcount != 1:
            raise ValueError(f"failed to update occurrence source_url: {item['occurrence_id']}")
...
def consistency_checks(conn, applied):
    ...
    for item in applied:
        ...
        if after.get("source_url") != item["new_source_url"]:
            issues.append(
                {
                    "severity": "high",
                    "issue_type": "source_url_not_applied",
                    "occurrence_id": item["occurrence_id"],
                    "expected": item["new_source_url"],
                    "actual": after.get("source_url") or "",
                }
            )
    return issues
...
def render_markdown(result):
    lines = [
        "# Reviewed missing source URLs apply report",
        "",
        ...
        f"- missing_source_url_count: {result['summary']['missing_source_url_count']}",
        "",
    ...
    for item in result["applied"]:
        lines.append(
            f"| {item['event_name']} | {item['old_source_url'] or '(none)'} | "
            f"{item['new_source_url']} | {item['reason']} |"
        )
    if result["skipped"]:
...
def run(args):
    ...
    with connect_existing(target_db) as conn:
        ...
        if args.apply and has_high_issue:
            ...
            rolled_back = False
            applied_out = applied
        missing_source_url_count = scalar(
            conn,
            "SELECT COUNT(*) FROM event_occurrences WHERE COALESCE(source_url, '') = ''",
        )
    ...
    result = {
        "generated_by": "apply_reviewed_missing_source_urls.py",
        "generated_at": now,
...
def main():
    ...
    print(
        "reviewed missing source URLs: "
        f"mode={result['mode']} "
        f"applied={result['summary']['applied_count']} "
        f"missing_source_url={result['summary']['missing_source_url_count']} "
