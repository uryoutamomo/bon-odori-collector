...
def test_low_priority_wiring_is_default_off_complete_and_ordered():
    ...
    assert workflow.index("Build low-priority review-inbox adapter inputs") < workflow.index("Dual-write low-priority queues to review inbox") < workflow.index("Commit low-priority inbox projection")
    assert "REVIEW_INBOX_READER_MODE: inbox" in workflow
    assert "REVIEW_INBOX_LEGACY_WRITER_ENABLED: 'false'" in workflow
    assert "git add -f data/accepted_venue_song_missing_venue_review.json" not in workflow
    assert "git add -f data/historical_reference_quality_review.json" not in workflow
    assert "python build_keyboard_review_ui.py" not in workflow
    ...
    assert gap["generated_by"] == "build_publication_gap_review.py"
    assert gap["rows"]
    assert len({row["gap_id"] for row in gap["rows"]}) == len(gap["rows"])
