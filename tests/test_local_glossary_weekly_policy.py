...
class LocalGlossaryWeeklyPolicyTest(unittest.TestCase):
    def test_launchagent_template_has_no_schedule(self):
        plist = (
            ROOT
            / "ops"
            / "com.ryotauchida.bon-odori.glossary-weekly.plist"
    ...
    def test_local_runner_requires_manual_flag(self):
        result = subprocess.run(
            [sys.executable, "run_weekly_glossary_review.py", "--days", "7"],
            cwd=ROOT,
            text=True,
            capture_output=True,
        )
    ...
    def test_runbook_documents_daily_owner_and_manual_review(self):
        doc = (ROOT / "docs" / "local-glossary-weekly-operations.md").read_text(
            encoding="utf-8"
        )

        self.assertIn("daily `collect.yml` flow", doc)
        self.assertIn("Human review of generated song/glossary queues", doc)
        self.assertIn("python3 run_weekly_glossary_review.py --manual --days 3", doc)
