...
from run_review_inbox_rare_signal_scheduled import CONFIRM, run_scheduled


FIXTURE = Path(__file__).parent / "fixtures" / "rare_signal_backcheck_two_examples.json"
JST = ZoneInfo("Asia/Tokyo")
ENABLED_ENV = {
    "REVIEW_INBOX_RARE_SIGNAL_SCHEDULED_ENABLED": "true",
...
def args_for(tmp: str, **overrides) -> Namespace:
    values = {
        ...
        "observation_id": "scheduled-123-1",
...
def test_enabled_scheduled_run_writes_all_items_and_audited_evidence():
    with tempfile.TemporaryDirectory() as tmp:
    ...
    assert len(rows) == 2
    assert {row["source_id"] for row in rows} == {"rare_signal"}
