...
class ComparePublicExportPostprocessorsTest(unittest.TestCase):
    ...
    def test_legacy_overlay_uses_report_safe_output_paths(self):
        ...
        self.assertEqual(commands[0][1:3], ["-m", "legacy.public_projection.apply_public_date_predictions"])
        self.assertEqual(commands[1][1:3], ["-m", "legacy.public_projection.apply_public_historical_references"])
        self.assertIn("--today", commands[1])
        self.assertIn("2026-07-16", commands[1])
        self.assertIn("--target-year", commands[1])
        self.assertEqual(commands[2][1:3], ["-m", "legacy.public_projection.apply_public_season_hints"])
